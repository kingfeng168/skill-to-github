...
> **注意**：替换含反斜杠的 Windows 路径时，不要用 raw string（`r"...\"` 结尾反斜杠会报 `SyntaxError: unterminated string literal`），改用普通字符串 `"C:\\Users\\<you>\\Desktop\\output\\"`（`\\` 表示一个反斜杠）。对 `references/*.py` 这类模板文件，直接整体重写为通用占位符（用正斜杠）比做字符串替换更稳。

脱敏后必须验证：
- `grep -rn "<your-username>\|<your-output-dir>\|.eia_key" <发布目录>` 应无残留（「如何配置自己的 key」类说明文字除外）。
...
**顺序必须是：建仓 → 推送 → Release**。空仓库无法建 Release（GitHub 返回 `422 "Repository is empty"`），所以要先 `git push` 让仓库有 commit，再建 Release。

用 token 调 GitHub REST API（**统一用 Python `urllib`，不要用 Windows 上的 curl**——curl 走 schannel 后端会报 `curl: (35) schannel: failed to receive handshake`，Python urllib 用 openssl 稳定）：

- 取用户名：`GET https://api.github.com/user`（`Authorization: token <TOKEN>`）
- 建仓库：`POST https://api.github.com/user/repos`，body `{"name":"<name>","description":"...","public":true}`（若返回 422 说明已存在，跳过即可）
